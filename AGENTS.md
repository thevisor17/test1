...
**Claude Code interop:** the personas in `agents/` work as Claude Code subagents (auto-discovered from this plugin's `agents/` directory) and as Agent Teams teammates (referenced by name when spawning). **Repo convention (not a platform hard-limit for subagents):** personas still must not invoke other personas — composition stays with slash commands or the user. On Claude Code, nested subagent spawn is possible (depth capped at 5); prevent it for a persona by omitting `Agent` from its `tools` allowlist or adding `Agent` to `disallowedTools`. **Still platform-enforced for Agent Teams:** teammates cannot spawn their own teammates (no nested teams). Plugin agents silently ignore the `hooks`, `mcpServers`, and `permissionMode` frontmatter fields.
